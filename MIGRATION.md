...
Version 0.2.0 renamed the agent-agnostic npm package from `pi-code-mode-mcp` to `@tmustier/code-mode-mcp`, and the repository, CLI, and MCP server to `code-mode-mcp`. The single public `exec` tool remains compatible and adds ranked in-code `search()` discovery.

## Upgrade

1. Install `@tmustier/code-mode-mcp@0.2.0`; its executable is `code-mode-mcp`.
2. Change the outer MCP command from `pi-code-mode-mcp` to `code-mode-mcp`, or change the pinned `npx` package name to `@tmustier/code-mode-mcp@0.2.0`.
...
The old npm package is deprecated and points to `@tmustier/code-mode-mcp`. GitHub redirects the previous repository URL.
